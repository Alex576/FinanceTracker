...
namespace FinanceTracker.Core.Services
{
    public class LayoutService : ILayoutService
    {
        ...
        public LayoutService(IServiceProvider serviceProvider, FinanceTrackerContext financeTrackerContext)
        {
        ...
        {
            var tools = await m_FinanceTrackerContext.Tools.Where(x => x.Id != (int)ToolCode.Settings).ToListAsync();
            ...
            var tiles = model.ToolFilter.Value is int toolValue ? await GetTileUnderTool(toolValue) : [];
        ...
        {
            return await m_FinanceTrackerContext.Tiles.Where(x => x.ToolCode == toolValue).Select(x => new Tile(x)).ToListAsync();
        }
        ...
        public async Task<LayoutEditor> GetLayoutEditor(ToolCode toolCode)
        {
            ...
            var layoutBuilder = new LayoutEditorBuilder();
            //var layoutTile = tiles.FirstOrDefault(t => t.Type == (int)TileTypeCode.Layout);
            //if (layoutTile != null)

            layoutEditor.LayoutItems = await GetLayoutEditorItems(await layoutBuilder.GetLayoutAsync(tiles, m_FinanceTrackerContext), filterBuilder);
            return layoutEditor;
        }
        ...
        private async Task<List<LayoutEntity>> GetLayoutEditorItems(LayoutPreview layoutPreview, FilterBuilder filterBuilder)
        {
            ...
            {
                var layout = await m_FinanceTrackerContext.Layouts.FirstOrDefaultAsync(x => x.TileCode == (int)previewItem.TileCode);
                //if (layout == null)
                //    continue;
                if (previewItem is FilterPreview filterPreview)
                {
                    var item = new LayoutEntity() { Type = TileTypeCode.Filter, TileCode = previewItem.TileCode };
                    if (layout != null)
                        item.Data = JsonConvert.DeserializeObject<FilterLayoutEntity>(layout.LayoutJson ?? "") ?? new();
                ...
                {
                    var item = new LayoutEntity() { Type = TileTypeCode.Grid, TileCode = previewItem.TileCode };
                    if (layout != null)
